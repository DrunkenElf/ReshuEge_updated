...
class MainMenu : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener,
    SubjectsFragment.OnFragmentInteractionListener, StartPageFragment.OnFragmentInteractionListener,
    SubjectsTheoryFragment.OnFragmentInteractionListener,
    SubjectSearchFragment.OnFragmentInteractionListener,
    SubjectThemesFragment.OnFragmentInteractionListener {
        ...
        when (id) {
            ...
            R.id.nav_stats -> {
                if (getNavController().currentDestination?.id != R.id.nav_subj_search && viewModel.peek() != "Статистика") {
                    ...
                    Log.d(
                        "Stats Nav1",
                        viewModel.peek() + " " + getNavController().currentDestination?.toString()
                    )
            ...
            R.id.nav_check_update -> {
                showUpdateMessage(
                    "Проверка обновлений",
                    "Проверка обновлений может занять несколько минут. Продолжить?"
                )
            }
            R.id.nav_settings -> {
                viewModel.push("Настройки")
    ...
    override fun onFragmentInteraction(subject: String) {//subj as href
        ...
        when (viewModel.peek()) {
            ...
            "Каталог заданий" -> { //+
                val action =
                    SubjectThemesFragmentDirections.actionNavSubjThemesToThemesFragment(subject)
                getNavController().navigate(action)
                viewModel.setTitle("${href_to_subj(subject)}.${viewModel.peek()}")
            ...
            "Теория" -> {//hz
                ...
                Log.d("teor1", subject)
                val action =
                    SubjectsTheoryFragmentDirections.actionNavSubjTheoryToTheoryFragment(subject)
                viewModel.setTitle("${href_to_subj(subject)}.${viewModel.peek()}")
            ...
            "Учителю" -> { // donei
                ...
                if (session.sessionState == SessionState.anonymus) {
                    showAuthorizeMessage(
                        ...
                        "Для просмотра статистики нужно " + "авторизоваться. Перейти к авторизации?",
                        false
                    )
                } else {
                    if (Connection.hasConnection(this)) {
                        val action = SubjectSearchFragmentDirections.actionGlobalSettingsFragment3()
                        getNavController().navigate(action)
                        viewModel.setTitle("${href_to_subj(subject)}.${viewModel.peek()}")
            ...
            "Статистика" -> { //done
                ...
                if (session.sessionState == SessionState.anonymus) {
                    ...
                    if (Connection.hasConnection(this)) {
                        val downloadStatistics =
                            DownloadStatistics(this, subject, "session=" + session.session)
    ...
    fun setVersionPreferences(context: Context) {
        val subjPrefs = SettingsImp(context)

        val subject_prefix = resources.getStringArray(R.array.subjects_prefix)
        CoroutineScope(Dispatchers.IO).launch {
            for (i in subject_prefix.indices) {
                var check = subjPrefs.getSubjVersion(subject_prefix[i])
                if (check == -1) {
                    subjPrefs.saveSubjVersion(subject_prefix[i], 0)
                    Log.d("myLog", "Версия предмета " + subject_prefix[i] + " = 0")
                } else
                    Log.d("myLog", "Версия предмета " + subject_prefix[i] + " = " + check)
                check = subjPrefs.getSubjTheorVersion(subject_prefix[i])
                if (check != -1) {
                    subjPrefs.saveSubjTheorVersion(subject_prefix[i], 0)
